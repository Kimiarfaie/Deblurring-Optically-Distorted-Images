...
% Number of methods
methods = {NAFNet, Restormer, MPRNet, DRBNet, LaKDNet_LFDOF_s, LaKDNet_DPDD_l, LaKDNet_DPDD_s, LaKDNet_LFDPDD_s, Stripformer};
...
% Initialize a cell array to store outlier information
outliersData = struct();
...
for i = 1:length(files)
    ...
    xlabel('Methods');
    ax = gca();
    outlierHandles = findobj(ax.Children.Children, 'Tag', 'Outliers');
    allOutlierValues = [];
    ...
    allOutlierValues = unique(allOutlierValues);

    % Store outliers for current metric
    for outlierValue = allOutlierValues
        outlierIndices = find(allData == outlierValue);
        for k = 1:numel(outlierIndices)
            ...
            if col <= numel(methods) && row <= numel(imageNames)
                ...
                if isfield(outliersData, metricNames{i})
                    outliersData.(metricNames{i}) = [outliersData.(metricNames{i}); {methods{col}, imageNames{row}}];
                else
                    outliersData.(metricNames{i}) = {methods{col}, imageNames{row}};
                end
            end
        end
    end
...
% Analyze collected data to find common outliers
summaryResults = 'Outlier Summary:\n';
metrics = fieldnames(outliersData);
for i = 1:numel(metrics)
    summaryResults = [summaryResults, sprintf('\n%s:\n', metrics{i})];
    metricData = outliersData.(metrics{i});
    if ~isempty(metricData)
        [uniqueRows, ~, idx] = unique(metricData);
        counts = accumarray(idx, 1);
        for j = 1:length(uniqueRows)
            summaryResults = [summaryResults, sprintf('%s appears %d times\n', uniqueRows{j}, counts(j))];
        end
    end
end
